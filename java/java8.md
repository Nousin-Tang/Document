...
目录 Java8 新特性Lambda 表达式什么是 Lambda ？Lambda 表达式定义格式使用条件注意点：Lambda 异常处理方法引用函数式接口（Functional Interface）Java8内置函数式接口(java.util.function)Stream示例常用方法操作新的日期时间 APIOptional 类方法引用函数式接口接口支持默认方法
...
- lambda 表达式只能引用标记了 final 的外层局部变量，这就是说不能在 lambda 内部修改定义在域外的局部变量，否则会编译错误。如果未标记final ，会将变量当成 final 的。
...
上面的代码当i为0时会引发ArithmeticException异常。通常的做法是通过try…catch来处理异常，防止系统崩溃。
