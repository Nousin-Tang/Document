...
/* 如何定义一个函数呢？
     ...
     2，在明确该定义功能的过程中是否需要未知的内容参与运算

           因为是在明确函数的参数列表（参数的类型个和个数）*/
...
//冒泡排序：相邻的两个元素进行比较，如果符合条件则换位
    ...
    public static void bubbleSort(int[] arr){
       ...
       for(int  x=0;x<arr.length;x++){
           ...
           for(int y=0;y<arr.length-x-1;y++){//因为随着循环次数的增加，每循环一次数组中要比较的元素就减少一个,且数组最后一位不用作比较。同时还可以避免角标越界
